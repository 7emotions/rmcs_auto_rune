 ...
 */
FeatureNode_ptr forceMakeCenterByInactiveFan(
    vector<FeatureNode_ptr>& inactive_fans, const vector<FeatureNode_ptr>& inactive_targets) {
 ...
 *       3. 仅用于临时测试使用
 */
FeatureNode_ptr forceMakeCenter(
    const vector<FeatureNode_ptr>& inactive_targets, const vector<FeatureNode_ptr>& active_targets,
    ...
    if (fans.size() >= 2) {
        ...
        if (!fans_intersection.empty()) {
            ...
            fans_center = std::make_unique<Point2f>(
                sum.x / static_cast<float>(fans_intersection.size()),
                sum.y / static_cast<float>(fans_intersection.size()));
        }
    }
    Point2f center{0, 0};
    ...
    if (targets_center == nullptr)
        ...
        center = ((*targets_center) * static_cast<float>(targets.size())
                  + (*fans_center) * static_cast<float>(fans.size()))
...
void RuneDetector::extractRuneFeatures(
    vector<FeatureNode_ptr>& targets_inactive, vector<FeatureNode_ptr>& targets_active,
    vector<FeatureNode_ptr>& fans_inactive, vector<FeatureNode_ptr>& fans_active,
    vector<FeatureNode_ptr>& centers, const vector<Contour_cptr>& contours,
    const vector<Vec4i>& hierarchy, unordered_set<size_t>& continue_idx) {
    {
        unordered_map<FeatureNode_cptr, unordered_set<size_t>> used_idxs{};
        RuneTarget::find_inactive_targets(
            targets_inactive, contours, hierarchy, continue_idx, used_idxs);
    ...
    {
        unordered_map<FeatureNode_cptr, unordered_set<size_t>> used_idxs{};
        RuneTarget::find_active_targets(
            targets_active, contours, hierarchy, continue_idx, used_idxs);
    ...
    }
    {
        ...
        targets_inactive_temp.insert(
            targets_inactive_temp.end(), targets_inactive.begin(), targets_inactive.end());
        RuneFan::find_inactive_fans(
            fans_inactive, contours, hierarchy, continue_idx, used_idxs, targets_inactive_temp);
...
bool RuneDetector::findFeatures(
    Mat src, vector<FeatureNode_cptr>& features, std::vector<RuneFeatureCombo>& matched_features) {
    ...
    for (size_t i = 0; i < contours.size(); i++) // 删除面积过小的轮廓
    {
        ...
        if (area < rune_detector_param.MIN_CONTOUR_AREA
            || area > rune_detector_param.MAX_CONTOUR_AREA) {
    ...
    extractRuneFeatures(
        targets_inactive, targets_active, fans_inactive, fans_active, centers, contours, hierarchy,
        continue_idx);

    if (targets_inactive.empty() && targets_active.empty() && fans_inactive.empty()
        && fans_active.empty())                  // 神符特征为空、放弃构建
        return false;
    // 判断是否需要强制构造神符中心
    bool is_force_make_center = [&]() -> bool {
        ...
        // 神符中心不可靠的范围比例
        double rune_center_unreliable_ratio =
        ...
        for (const auto& center : centers) {
            ...
            if (center_pos.x < left_x || center_pos.x > right_x || center_pos.y < top_y
                || center_pos.y > bottom_y) {
    ...
    FeatureNode_ptr rune_inactive_fan =
        fans_inactive.empty() ? nullptr : fans_inactive[0]; // 未激活扇叶

    RuneFanInactive::correctDirection(
        rune_inactive_fan, getCenter(rune_center));         // 未激活扇叶的方向矫正
    ...
    RuneDetector::filterFarContours(
        contours, rune_targets, rune_center, rune_fans, continue_idx,
        far_contour_idxs);                                  // 过滤距离过远的轮廓
    continue_idx.insert(far_contour_idxs.begin(), far_contour_idxs.end());

    Point2f rotate_center{};
    if (RuneDetector::getRotateCenter(rune_targets, rune_fans, rotate_center)
        == false)                                           // 获取旋转中心
    ...
    RuneFan::find_incomplete_active_fans(
        fans_active_incomplete, contours, hierarchy, continue_idx, rotate_center,
        fans_active_incomplete_idxs);                            // 未完整的已激活扇叶
    filterActiveFanIncomplete(
        fans_active_incomplete, targets_inactive, targets_active, fans_inactive, fans_active,
        rune_center);                                            // 筛选未完整的已激活扇叶
    rune_fans.insert(rune_fans.end(), fans_active_incomplete.begin(), fans_active_incomplete.end());

    auto temp_matched_features =
        getMatchedFeature(rune_targets, rune_center, rune_fans); // 获取配对后的特征组
