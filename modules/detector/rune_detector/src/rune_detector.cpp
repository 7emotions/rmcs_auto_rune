 ...
 */
static inline void setFeatures(
    const std::vector<RuneFeatureComboConst>& matched_features,
    std::vector<FeatureNode_cptr>& features) {
    // 更新特征组
 ...
 * @brief 设置序列组的基础属性
 */
inline static void setBaseProperties(
    std::vector<FeatureNode_ptr>& groups, const GyroData& gyro_data, int64 record_time) {
...
void RuneDetector::detect(DetectorInput& input, DetectorOutput& output) {
    ...
    auto updateRuneGroup = [&]() -> bool {
        ...
        // 尝试获取所有的神符组合体
        if (!getRunes(
                current_combos, rune_group, to_const(matched_features),
                rune_group->getPoseCache().getPoseNodes()[CoordFrame::CAMERA]))
            return false;
    ...
    auto updateRuneGroupVanish = [&]() -> bool {
        ...
        PoseNode runeGroup_to_cam;
        if (!rune_group->getCamPnpDataFromPast(runeGroup_to_cam, 0))
            return false;
        ...
        // 尝试获取所有的神符组合体
        if (!getRunes(
                current_combos, rune_group, rune_group->getLastFrameFeatures(),
                rune_group->getPoseCache().getPoseNodes()[CoordFrame::CAMERA]))
            return false;
