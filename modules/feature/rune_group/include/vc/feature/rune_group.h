...
class RuneGroup : public TrackingFeatureNode {
    ...
    DEFINE_PROPERTY_WITH_INIT(
        RawDatas, public, protected, (std::deque<double>), {});         //!< 原始角度数据
    DEFINE_PROPERTY_WITH_INIT(
        AngleSpeeds, public, protected, (std::deque<double>), {});      //!< 角速度数据（顺负逆正）
    DEFINE_PROPERTY(
        PredictFunc, public, public, (std::function<double(int64_t)>)); //!< 角度变化预测函数
    DEFINE_PROPERTY(
        RecommendedTicks, public, public, (std::vector<int64_t>));      //!< 推荐击打时间戳列表
    DEFINE_PROPERTY(
        CamToGyro, public, protected, (PoseNode)); //!< 相机坐标系到陀螺仪坐标系的变换位姿
    ...
    DEFINE_PROPERTY(
        LastValidPoseGyro, public, protected, (PoseNode)); //!< 最后一次有效观测位姿（陀螺仪坐标系）
...
public:
     ...
     */
    static inline std::shared_ptr<RuneGroup> make_feature() {
        return std::make_shared<RuneGroup>();
    }
     ...
     * @param[out] runeGroup_to_cam 输出相机坐标系PNP数据
     * @param[in] dt_ms 预测时间间隔（毫秒）
     * @return 是否预测成功
     */
    bool getCamPnpDataFromPast(PoseNode& runeGroup_to_cam, int64_t dt_ms) const;
     ...
     */
    virtual void
        drawFeature(cv::Mat& image, const DrawConfig_cptr& config = nullptr) const override;
...
private:
    std::unordered_map<std::string, std::shared_ptr<DataConverter>>
        __data_converters;       //!< 数据转换器映射

    void updatePnpData(const PoseNode& rune_to_gyro, const GyroData& gyro_data); //!< 更新PNP数据
    void clearPreviousData();                                                    //!< 清除上一帧数据
    bool correctRoll(
        const PoseNode& raw_cam_pnp_data, PoseNode& corrected_cam_pnp_data);     //!< 修正roll角
    PoseNode _correntRoll_last_cam_pnp_data{};        //!< 上一帧相机PNP数据
    bool _correntRoll_last_cam_pnp_data_flag = false; //!< 数据有效标志
    static bool checkExtremePose(
        const PoseNode& rune_to_cam, const PoseNode& rune_to_gyro,
        const GyroData& gyro_data);                   //!< 检测极端位姿
