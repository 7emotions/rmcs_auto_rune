...
inline bool checkPoseDiff(const Matx61f& p1, const Matx61f& p2) {
    ...
    for (int i = 0; i < 6; i++)
        diff[i] = p1(i) - p2(i);
    const double limits[6] = {
        rune_group_param.MAX_X_DEVIATION,     rune_group_param.MAX_Y_DEVIATION,
...
bool RuneGroup::checkExtremePose(
    const PoseNode& r_cam, const PoseNode& r_gyro, const GyroData& gyro) {
    ...
    float yaw = pos_cam(3);
    if (yaw > rune_group_param.MAX_YAW_DEVIATION_ANGLE
        || yaw < -rune_group_param.MAX_YAW_DEVIATION_ANGLE) {
    ...
    if (pitch > rune_group_param.MAX_PITCH_DEVIATION_ANGLE
        || pitch < -rune_group_param.MAX_PITCH_DEVIATION_ANGLE) {
...
bool RuneGroup::getCamPnpDataFromPast(PoseNode& rune_to_cam, int64_t dt_ms) const {
    PoseNode pos_to_cam;
    ...
    if (isSetPredictFunc() && getPredictFunc()) {
        ...
        if (ticks.size() >= 2) {
            float angle = getPredictFunc()(ticks.front() + dt_ms);
...
inline unordered_set<size_t>
    getAbnormalFrames(const vector<TrackingFeatureNode_cptr>& trackers, size_t range) {
    ...
    for (size_t f = 0; f < range; f++)
        for (size_t t = 0; t < trackers.size(); t++) {
            auto combo =
                RuneCombo::cast(TrackingFeatureNode::cast(trackers[t])->getHistoryNodes()[f]);
            types[f][t] = (!combo || combo->getRuneType() == RuneType::UNKNOWN)
                            ? RuneType::UNKNOWN
                            : combo->getRuneType();
        }
...
inline void
    drawPentagonalPrism(cv::Mat& img, const PoseNode& p, float radius, float height, cv::Scalar c) {
