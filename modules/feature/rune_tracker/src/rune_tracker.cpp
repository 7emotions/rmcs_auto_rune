...
void RuneTracker::updateFromRune(FeatureNode_ptr p_combo) {
    ...
    if (m_prev_tick > 0 && (m_center_ekf.isInitialized() || m_target_ekf.isInitialized())) {
        double dt = static_cast<double>(current_tick - m_prev_tick);
    ...
    if (child_features.count(FeatureNode::ChildFeatureType::RUNE_CENTER) > 0) {
        ...
        if (pose_nodes.count(CoordFrame::CAMERA) > 0) {
            ...
            m_center_ekf.update(Vec3f(
                static_cast<float>(tvec[0]), static_cast<float>(tvec[1]),
    ...
    if (child_features.count(FeatureNode::ChildFeatureType::RUNE_CENTER) > 0
        && child_features.count(FeatureNode::ChildFeatureType::RUNE_TARGET) > 0) {
        const auto& center = child_features.at(FeatureNode::ChildFeatureType::RUNE_CENTER);
        const auto& target = child_features.at(FeatureNode::ChildFeatureType::RUNE_TARGET);
        const auto& center_pose_nodes = center->getPoseCache().getPoseNodes();
        const auto& target_pose_nodes = target->getPoseCache().getPoseNodes();

        if (center_pose_nodes.count(CoordFrame::CAMERA) > 0
            && target_pose_nodes.count(CoordFrame::CAMERA) > 0) {
            const auto& center_tvec = center_pose_nodes.at(CoordFrame::CAMERA).tvec();
            const auto& target_tvec = target_pose_nodes.at(CoordFrame::CAMERA).tvec();

            const float dx = static_cast<float>(target_tvec[0] - center_tvec[0]);
            const float dy = static_cast<float>(target_tvec[1] - center_tvec[1]);
            m_current_radius = std::sqrt(dx * dx + dy * dy);
            m_current_target_z = static_cast<float>(target_tvec[2]);
...
bool RuneTracker::isPredictionStable() const {
    return m_prediction_unstable_cooldown == 0
        && m_prediction_stable_frames >= rune_tracker_param.PREDICTION_STABLE_MIN_SUCCESS_FRAMES
        && getHistoryNodes().size()
               >= static_cast<size_t>(rune_tracker_param.PREDICTION_STABLE_MIN_HISTORY)
        && isCenterEKFInitialized() && isTargetEKFInitialized() && isCircularEKFInitialized()
        && m_current_radius > 1e-3f && getDropFrameCount() == 0;
}

void RuneTracker::markPredictionUnstable(int cooldown_frames) {
    m_prediction_stable_frames = 0;
    const int effective_cooldown = cooldown_frames >= 0
                                     ? cooldown_frames
                                     : rune_tracker_param.PREDICTION_UNSTABLE_COOLDOWN_FRAMES;
    m_prediction_unstable_cooldown = std::max(m_prediction_unstable_cooldown, effective_cooldown);
}

void RuneTracker::advancePredictionStability() {
    if (!isCenterEKFInitialized() || !isTargetEKFInitialized() || !isCircularEKFInitialized()
        || getHistoryNodes().size()
               < static_cast<size_t>(rune_tracker_param.PREDICTION_STABLE_MIN_HISTORY)
        || m_current_radius <= 1e-3f) {
        m_prediction_stable_frames = 0;
        return;
    }

    if (m_prediction_unstable_cooldown > 0) {
        --m_prediction_unstable_cooldown;
        m_prediction_stable_frames = 0;
        return;
    }

    ++m_prediction_stable_frames;
}

void RuneTracker::resetTrackingState() {
    m_center_ekf.reset();
    m_target_ekf.reset();
    m_circular_ekf.reset();
    m_current_radius = 0.0f;
    m_current_target_z = 0.0f;
    m_prev_tick = 0;
    m_prediction_stable_frames = 0;
    m_prediction_unstable_cooldown = 0;
    getDropFrameCount() = 0;
}
...
void RuneTracker::updateVisible(bool is_visible) {
    ...
    if (is_visible) {
        __vanish_num = 0;
        advancePredictionStability();
    } else {
        ++__vanish_num;
        markPredictionUnstable();
    }
}
...
cv::Point3f RuneTracker::getPredictedTargetPos(double dt_sec) const {
    ...
    if (m_circular_ekf.isInitialized() && m_current_radius > 1e-3f) {
        ...
        return m_circular_ekf.predictAheadPos(
            dt_sec, predicted_center, m_current_radius, m_current_target_z);
    }
...
void RuneTracker::setMotionMode(bool is_sinusoidal) {
    const auto target_mode =
        is_sinusoidal ? RuneCircularEKF::Mode::SINUSOIDAL : RuneCircularEKF::Mode::CONSTANT;
    if (m_circular_ekf.getMode() == target_mode)
        return;

    m_circular_ekf.setMode(target_mode);
    resetTrackingState();
    markPredictionUnstable();
}

inline void drawPentagonWedge(
    cv::Mat& img, const PoseNode& p, float radius, float height, int sector_index = 0,
    cv::Point2f center_offset = cv::Point2f(0.0f, 0.0f), cv::Scalar c = cv::Scalar(0, 255, 0)) {
    ...
    pts3d.emplace_back(
        center_offset.x + radius * std::cos(angle1), center_offset.y + radius * std::sin(angle1),
        -hz);                                                  // 1: 底顶点1
    pts3d.emplace_back(
        center_offset.x + radius * std::cos(angle2), center_offset.y + radius * std::sin(angle2),
        -hz);                                                  // 2: 底顶点2
    // 上底（三角形 z = +hz）
    ...
    projectPoints(
        pts3d, p.rvec(), p.tvec(), camera_param.cameraMatrix, camera_param.distCoeff, pts2d);
...
inline void
    drawCube(cv::Mat& img, const PoseNode& p, float x_len, float y_len, float z_len, cv::Scalar c) {
