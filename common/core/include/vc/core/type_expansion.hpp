...
public:
     ...
     */
    bool contains(T value) const { return value >= low && value <= high; }
     ...
     * @brief 打印区间信息
     */
    void print() const { std::cout << "[" << low << ", " << high << "]" << std::endl; }
     ...
     */
    T operator-(const ArithmeticRange<T>& other) const { return this->low - other.high; }
};
